...
# Check system
for X in 'basename' 'cat' 'echo' 'ls' 'lz4' 'read' "${READLINK}" 'tar'
do
...
then
	...
	NEW_BACKUP_FILE="${OUTDIR}/${DIRNAME}.tar.lz4"
	...
	#echo tar cvfp - "${DIRNAME}" | lz4 -zc >"${NEW_BACKUP_FILE}" 2>"${ERRLOG}"
	if tar cvfp - "${DIRNAME}" | lz4 -z - "${NEW_BACKUP_FILE}" 2>"${ERRLOG}"
	then
		cat "${ERRLOG}"
...
then
	...
	NEW_BACKUP_FILE="${OUTDIR}/${FILENAME}.tar.lz4"
	check_delete_file "${NEW_BACKUP_FILE}"
	...
	then
		cat "${ERRLOG}"
		[[ -f "${NEW_BACKUP_FILE}" ]] && ls -lh "${NEW_BACKUP_FILE}"
	else
		cat "${ERRLOG}"
		[[ -f "${NEW_BACKUP_FILE}" ]] && rm -fv "${NEW_BACKUP_FILE}"
	fi
else
